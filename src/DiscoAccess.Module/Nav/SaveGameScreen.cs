...
{
    /// <summary>
    /// The save menu: the save grid whose primary action saves over the focused slot. The first row is the
    ...
    /// </summary>
    public sealed class SaveGameScreen : SaveLoadGridScreen
    {
        ...
        // an existing save). The grid allows this shorter-elsewhere row: out-of-range cells read as empty.
        protected override void BuildRow(IModHost host, Grid grid, SaveGameListEntry entry, Selectable row, Button primary, Button delete)
        {
            if (entry.IsNewSave)
                grid.AddRow(
