    ...
    internal sealed class QuicktravelCell : UIElement
    {
        ...
        public override string GetFocusText()
        {
            if (_button.CheckTequilaInActivationRadius())
                return WhirlingInWords.Core.Text.SpokenLine.Join(_name, Strings.JournalYouAreHere);
            if (_button.wasVisited)
                return WhirlingInWords.Core.Text.SpokenLine.Join(_name, Strings.JournalVisited);
            return _name;
        }
