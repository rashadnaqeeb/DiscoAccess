...
namespace DiscoAccess.Core.UI
{
    ...
    /// for a consumable, the pawn value when positive - then what the item does (its effects) and its
    /// description. The effects and description ride on the item itself rather than a separate detail stop,
    ...
    public static class InventoryItemAnnouncer
    {
        public static string Compose(InventoryItemState s)
        {
            ...
            if (s.Value > 0) parts.Add(Strings.Strings.ItemValue(s.Value));
