...
namespace WhirlingInWords.Dev
{
    /// <summary>
    /// In-process dev driver, on by default (set WHIRLING_NO_DEV=1 to disable). The HTTP server
    ...
    {
        public const string DisableEnv = "WHIRLING_NO_DEV";
        public const string PortEnv = "WHIRLING_DEV_PORT";
        ...
        /// <summary>Stand up the loopback server unless WHIRLING_NO_DEV=1.</summary>
        public void Start()
        {
            if (Environment.GetEnvironmentVariable(DisableEnv) == "1")
            {
                _log.LogInfo("Dev server disabled (WHIRLING_NO_DEV=1)");
                return;
            }
