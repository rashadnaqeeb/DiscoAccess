...
using WhirlingInWords.Core.Speech;
using WhirlingInWords.Core.Strings;
using WhirlingInWords.Dev;
using WhirlingInWords.Host;
using WhirlingInWords.Modularity;
using WhirlingInWords.Speech;
using Il2CppInterop.Runtime.Injection;

namespace WhirlingInWords
{
    /// <summary>
    ...
    /// reloadable module. No feature logic lives here - that's in WhirlingInWords.Module.
    /// </summary>
    [BepInPlugin(Guid, Name, BuildVersion.Value)]
    public sealed class Plugin : BasePlugin
    {
        public const string Guid = "com.rashad.whirlinginwords";
        public const string Name = "Whirling in Words";
        ...
        public override void Load()
        {
            ...
            // Headless/overnight dev runs set WHIRLING_NO_SPEECH=1 so an unattended session doesn't
            ...
            bool muteSpeech = System.Environment.GetEnvironmentVariable("WHIRLING_NO_SPEECH") == "1";
            _prism = new PrismBackend(Log);
            if (muteSpeech)
                Log.LogInfo("Speech muted (WHIRLING_NO_SPEECH=1); spoken text still captured for /speech");
            else
                _prism.Initialize();
            ...
            string modulePath = Path.Combine(pluginDir, "WhirlingInWords.Module.dll");
